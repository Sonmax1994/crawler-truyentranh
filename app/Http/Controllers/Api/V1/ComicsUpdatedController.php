...
class ComicsUpdatedController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        try {
            if (config('comic.mock_api_enable')) {
                return File::json(config_path('mock_json/comics_updated.json'));
            }
            ...
            $comics = Cache::remember($cacheKey, $this->cacheSeconds, function () use ($inputs) {
                $comics = $this->comicServices->getLatestComics($inputs);
